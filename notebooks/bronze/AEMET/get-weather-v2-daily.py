...
for year in years:
    ...
    for comunidad, g in y_df.groupby("comunidad"):
        out = g.drop(columns=["comunidad"])
        # Reordenamos columnas para legibilidad
        cols_order = ["año", "mes", "dia", "fecha", "tmax", "tmin", "tmed", "prec", "viento_max"]
        out = out[[c for c in cols_order if c in out.columns]]

        # Aseguramos fecha como string ISO (no epoch)
...
print("¡Archivos JSON DIARIOS guardados por CCAA y año!")
