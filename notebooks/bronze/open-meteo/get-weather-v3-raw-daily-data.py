...
# ===== 1) Config robusta =====
GROUP_SIZE = 3                  # <= más pequeño para evitar timeouts
MAX_RETRIES = 5                 # reintentos de red
INITIAL_BACKOFF = 1.5           # s
CONNECT_TIMEOUT = 10            # s
READ_TIMEOUT = 120              # s
PAUSE_BETWEEN_CALLS = 0.5       # s, cortesía a la API
...
# ===== 5) Descarga por MESES (robusto) y guardado ANUAL por CCAA — SIN LIMPIEZA =====
ccaa_list = list(ccaa_coords.items())
years = [2023, 2024, 2025]

# Carpeta base en Lakehouse (BRONZE)
base_dir = "/lakehouse/default/Files/bronze/AEMET/brz_ccaa_diario_2023_2025"
os.makedirs(base_dir, exist_ok=True)

# Acumulador: dict[(year, comunidad)] -> list[rows]
bucket = {}
    ...
    for m_start, m_end in iter_months(y_start, y_end):
        ...
        for group in chunks(ccaa_list, GROUP_SIZE):
            ...
            for (name, _), loc in zip([*group], locations):
                ...
                # Bronze: SIN limpieza ni redondeos; nombres originales de la API
                rows = []
                ...
                if key not in bucket:
                    bucket[key] = []
                bucket[key].extend(rows)

# ===== 6) Escritura ANUAL por CCAA (un JSON por comunidad y año) =====
for (year, comunidad), rows in bucket.items():
...
print("¡Guardado en lkh_iemc/Files/bronze/AEMET/brz_ccaa_diario_2023_2025/<año>/brz_<comunidad>_<año>.json (SIN LIMPIEZA)!")
